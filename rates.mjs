// Carriage Capital — daily market-rate fetcher (Netlify Function)
...
// update that spread periodically, or replace with a licensed dealer feed when available.
...
export default async () => {
  ...
  try {
    ...
    const head = lines[0];
    const row = lines[1], prev = lines[2] || lines[1];        // newest-first
  ...
  // --- Derived: 10-year swap estimate (Treasury 10Y + indicative spread) ---
  const ust10 = rates.find((r) => r.label === 'UST 10-Year');
  if (ust10) {
    rates.push({ label: '10-Yr Swap (≈)', value: +(ust10.value + SWAP_SPREAD_10Y).toFixed(2), change: ust10.change });
  }

  const asOf = new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
  return new Response(JSON.stringify({ asOf, rates }), {
